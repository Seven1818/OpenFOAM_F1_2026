...
SCRIPT_DIR = Path(__file__).parent  # always PostProcesser/, regardless of CWD

def discover_plotters(folder="functions") -> dict:
    functions_dir = SCRIPT_DIR / folder # Fix added to find the function file it's not linked to where RUnner.py is :)
    registry = {}
    for f in functions_dir.glob("*.py"):
        if f.stem == "__init__":
            continue
        source = f.read_text()
        if "PostStage" not in source:
            continue
        mod = importlib.import_module(f"functions.{f.stem}")
        for attr_name in dir(mod):
            obj = getattr(mod, attr_name)
            try:
                if issubclass(obj, PostStage) and obj is not PostStage:
                    registry[obj.name] = obj
            except TypeError:
                continue
    return registry

PLOTTER_REGISTRY = discover_plotters()
