...
class SidePodPressurePlotter(PostStage): #Class object that inherits from the PostStage base class, responsible for generating pressure plots inside sidepods from OpenFOAM simulation data
    name = "sidepod_pressure" #this name needs to match the name in the config.yaml!
    ...
    def run(self) -> list[Path]:
        ...
        file_path = self.case_dir / self.config.get("file", "postProcessing/probes/0/p") # constructs the file path to the side pod data file by combining the case directory with the relative path specified in the configuration, defaulting to "postProcessing/sidePod/0/sidePod.dat" if no specific file path is provided in the configuration
