...
class YPlusPlotter(PostStage): #Class object that inherits from the PostStage base class, responsible for generating Y+ plots from OpenFOAM simulation data
    ...
    def run(self) -> list[Path]:
        ...
        file_path = self.case_dir / self.config.get("file", "postProcessing/yPlus/0/yPlus.dat") # constructs the file path to the Y+ data file by combining the case directory with the relative path specified in the configuration, defaulting to "postProcessing/yPlus/0/yPlus.dat" if no specific file path is provided in the configuration
